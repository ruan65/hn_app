...
void main() {
  test('parses topstories.json', () {
    ...
    expect(parseStoriesIds(jsonString) is List<int>, true);
    expect(parseStoriesIds(jsonString).first, 19275315);
    expect(parseStoriesIds(jsonString).last, 19271248);
    const emptyListStr = '[]';
    expect(parseStoriesIds(emptyListStr), []);
  });
  ...
  test('parses item.json over network', () async {
    ...
    if (response.statusCode == 200) {
      final idList = parseStoriesIds(response.body);
